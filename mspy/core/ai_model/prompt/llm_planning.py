...
from ...types import ActionSpaceItem
from .common import bbox_description


def description_for_action(
    action: ActionSpaceItem, locator_schema_type_description: str
) -> str:
    fields = [f'- type: "{action.name}"']
    if action.param_hint:
        fields.append(f"- param: {action.param_hint}")
...
def system_prompt_to_task_planning(
    ...
    if include_bbox and not vl_mode:
        raise ValueError("vlMode cannot be undefined when includeBbox is true.")

    locator_desc = bbox_description(vl_mode if include_bbox else None)
    action_description_list = [
        description_for_action(action, locator_desc) for action in action_space
    ]
...
"""
    ...
    common_output_fields = (
        '"error"?: string, // Error messages about unexpected situations, if any. '
        'Only think it is an error when the situation is not foreseeable according to the instruction. '
        'Use the same language as the user\'s instruction.\n'
        '  "more_actions_needed_by_instruction": boolean, // Consider if there is still more action(s) to do '
        'after the action in "Log" is done, according to the instruction. If so, set this field to true. '
        'Otherwise, set it to false.'
    )

    bbox_hint = (
        f', "bbox": [100, 200, 300, 400]' if vl_mode and include_bbox else ""
    )
...
{{
  ...
  "action": {{
    ...
    "param": {{
      "locate": {{ 
        "prompt": "The login button"{bbox_hint}
      }}
    }}
  }}
}}
